...
class Normalize(object):
    ...
    def __call__(self, image, mask, depth=None, edge=None):
        image = (image - self.mean) / self.std
        mask /= 255
        if depth is None and edge is None:
            return image, mask
        else:
            depth /= 255
            edge /= 255
...
class RandomCrop(object):
    def __call__(self, image, mask, depth, edge):
        H, W, _ = image.shape
        ...
        return image[p0:p1, p2:p3, :], mask[p0:p1, p2:p3], depth[p0:p1, p2:p3], edge[p0:p1, p2:p3]


class RandomFlip(object):
    def __call__(self, image, mask, depth, edge):
        if np.random.randint(2) == 0:
            return image[:, ::-1, :], mask[:, ::-1], depth[:, ::-1], edge[:, ::-1]
        else:
            return image, mask, depth, edge
...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.mode == 'train':
            ...
            image, mask, depth, edge = self.normalize(image, mask, depth, edge)
            image, mask, depth, edge = self.randomcrop(image, mask, depth, edge)
            image, mask, depth, edge = self.randomflip(image, mask, depth, edge)
            return image, mask, depth, edge
        else:
    ...
    def collate(self, batch):
        ...
        image, mask, depth, edge = [list(item) for item in zip(*batch)]
